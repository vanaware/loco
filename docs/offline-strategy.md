...
O Loco é um PWA de mensagens **sem servidor central**. Isso significa que o app deve continuar funcionando mesmo quando:
...
A estratégia de offline do Loco é simples: **tudo que é essencial para o usuário deve estar no dispositivo dele**. Comunicação online acontece apenas quando possível, e sempre priorizando canais P2P.
...
3. **Push como fallback**: usar Web Push apenas quando o destinatário não está online.
4. **PWA como plataforma**: Service Worker, Cache API e armazenamento persistente tornam o app resilientes.
...
Essas operações dependem de conectividade, mas o app deve continuar funcional e sincronizar quando possível.
...
O app nunca bloqueia o usuário. A mensagem é salva imediatamente e o envio tentado em background.
...
- **Funciona offline em LAN**: dois dispositivos na mesma rede podem se comunicar via P2P mesmo sem internet.
...
Quando o P2P não funciona, o Web Push é usado para acordar o dispositivo do destinatário. Ele é considerado fallback porque:
...
Mesmo assim, é essencial porque permite alcançar contatos que estão com o navegador fechado ou com o dispositivo inativo.

## Service Worker e cache

O Service Worker (`src/sw.ts`) é responsável por:
...
- **Cache First**: scripts, estilos, imagens e fontes são servidos do cache se disponíveis.
- **Network First**: HTML sempre tenta a versão mais recente.
- **Background Sync**: marca atualizações pendentes para sincronizar quando online.
...
- A mensagem é salva localmente e enviada automaticamente quando o avião pousar e houver conexão.
...
A estratégia offline do Loco segue a premissa de que o app é **independente e resiliente**:
...
- Service Worker e Cache API garantem que o app funcione sem internet.
- Sincronização automática acontece quando a conectividade retorna.

Esse design coloca o máximo de controle possível nas mãos do usuário, sem depender de infraestrutura central.
